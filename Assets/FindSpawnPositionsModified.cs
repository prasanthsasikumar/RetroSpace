...
public class FindSpawnPositionsModified : MonoBehaviour
{
    ...
    public void SpawnAtCenterOfNearestTable()
    {
        if (MRUK.Instance)
        {
            ...
            {    
                ...
                if (table)
                {
                    MRUKAnchor tableAnchor = table;
                    // Get the center of the table
                    var center = tableAnchor.transform.position;
                    if (SpawnObject.gameObject.scene.path == null)
                    {
                        Instantiate(SpawnObject, Vector3.zero, Quaternion.identity, transform);
                    }
